...
computation over arbitrary rings and fields.  We present a basic
development of their model as well as a number of their results, the
most important of which is the Path Decomposition Theorem, which
provides a necessary condition for decidability of a set in terms of
its topological complexity. \todo{More on own work here?}
...
us to fully capture the classical model. \todo{last sentence feels clunky}
...
computable set can be constructed from a countable set of basic semi/quasi algebraic sets.
  ...
  \todo{section on $R^\infty$ and $R_\infty$?}
  ...
  \todo{fill in path decomposition theorem for infinite case}
...
\todo{Should new work from this term be in the same document or a different one?}
