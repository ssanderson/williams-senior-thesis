...
% \begin{frame}{Characterization of Halting Sets for FDMsn}
...
%   \begin{itemize}
%   \item[] Can show that $D$ is decidable if and only if $D_{yes}$ and
%     $D_{no}$ are the halting sets of some machine $M$.\pause
%   \item[] By above, this means if $D$ is decidable, then $D_{yes}$ and
%     $D_{no}$ are both countable unions of semi/quasi-algebraic
%     sets.\pause
%   \item[] Can be shown that $\mathcal{M}$ cannot be expressed as such
%     a union.
%   \end{itemize}
% \end{frame}
...
%   Hard to represent inputs from big polynomials in a finite state
%   space?
...
%   Create an infinitely long ``tape'' by letting $\statespace_M =
%   R_\infty$.\pause\\

%   Elements of $R_\infty$ look like:
%   $$\cdots, x_{-2}, x_{-1}, x_0 . x_1, x_2, \cdots$$\pause\\
  
%   Extend polynomial maps $g: R^m \rightarrow R^n$ to $\hat{g}:
...
%   (Image of Turing Machine Tape, graphic highlighting length of tape,
%   followed by highlighting single cell.  Note that the latter notion
%   doesn't make sense for classical theory because all symbols are
...
% \begin{frame}{Two Characterizations of Classical $\mathbf{NP}$}

%     PLACEHOLDER

%     (Define w/r/t verifiability, define w/r/t nondeterministic Turing Machine)
  
% \end{frame}

% \begin{frame}{Classical Nondeterminism}

%   PLACEHOLDER
  
%   (Two column slide, left side shows a straight-line sequence of
%   configurations, right side shows a branching tree of
%   configurations.  Explain rules for acceptance.)
