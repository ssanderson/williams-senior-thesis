...
%   A machine that acts as a decider for a decision problem $S$ is by
%   definition a recognizer for both $S_{yes}$ and $S_{no}$ .
%   Conversely, if $M$ and $M'$ are recognizers for $S$ and $R^n - S$,
%   we can contruct a decision machine for $S$ by simulating $M$ and
%   $M'$ in parallel, accepting if $M$ accepts and rejecting if $M'$
%   accepts.  Since one of $M$ and $M'$ must halt eventually, we are
%   guaranteed that our new machine will halt for all possible inputs,
%   and will accept an input if an only if it is in $S_{yes}$.
