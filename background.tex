...
Method, determining membership in the Mandelbrot Set, and the problem
of determining whether a finite set of polynomials in $n$
...
these containments it follows that if $\p \subset \ndetp$ or $\ndetp
...
\begin{definition}{Turing Machine}
  ...
  tape head to the left or right depending on the value of $d$.  In
  the case that the machine head would move left when on the leftmost
  cell, it stays in place.
  
\end{definition}

\textbf{Example: } The following Turing Machine reads in a binary
...
On the input $1001$ computation of the above machine would run as
follows:

\tape[0]{{1,0,0,1,\blank,$\cdots$}}
\tape[1]{{0,0,0,1,\blank,$\cdots$}}
\tape[2]{{0,1,0,1,\blank,$\cdots$}}
\tape[3]{{0,1,1,1,\blank,$\cdots$}}
\tape[4]{{0,1,1,0,\blank,$\cdots$}}
...
\begin{definition}{\textbf{Halting Computation and time-t Halting Set}}
  ...
  is a sequence of configurations: $$\compute_M(x) = (z_0, z_1, \ldots
  z_t)$$ such that \\
