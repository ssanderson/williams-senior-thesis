...
Method, determining membership in the Mandelbrot Set, and the Hilbert
Nullstellensatz, which asks whether a finite set of polynomials in $n$
variables share a common root. \\

To address the above issues and others, Blum et al. develop a theory
of computation that generalizes the Turing Machine model to
computation over arbitrary rings and fields.  In this thesis, we
...
generalization of the $\mathbf{NP}$.  Our major results are that
$\ndetp \subseteq \exptime$ over any ring or field and that $\ndetp =
\np$ for any finite ring or field (eg $\integers_p$).
...
heavily from \cite{B89} and \cite{B98}.  We begin with a brief review
of the classical Turing Machine model.  For a more thorough reference
see \cite{S06}.  For the original article introducing Turing Machines, 
see \cite{T36}
...
transition function which provides instructions for how the tape head
operates and how the machine transitions from state to state.  These
ideas are made precise as follows:
...
\begin{definition}{Turing Machine}
  ...
  \item $\Sigma$ is an alphabet of input characters. 
  \item $\Gamma \subset \Sigma$ is the tape alphabet.
  ...
  \times Q \times \naturals$ encoding the symbols currently written on the tape,
  the machine's current state, and the position of the tape head.  At
  each computation step, the machine read the symbol, $\sigma$ on the
  cell currently occupied by the tape head tape head rests.  If the
  machine is in state $q$, it then calculates $\delta(q, \sigma)$ =
  $(q', \sigma', d \in \set{\leftarrow, \rightarrow})$, transitions
  into state $q'$, writes $\sigma'$ to the current cell, and move the
  tape head to the left or right depending on the value of $d$.
...
can analyze their properties, we first present Blum et al's definition of
...
In what follows, let $R$ be a commutative ring with unit, and let $l,
...
with $(\eta_1, I_M(x)$ and follows the computing endomorphism of $M$,
halting upon reaching the output node of $M$.  The ``run time'' of a
computation is given by the length of this path, and the output value
of a computation is the result of $O_m$ called on whatever is left in
$\statespace_M$ when a terminal configuration is reached.
...
defines a unique next configuration $z' = (\beta, x')$ as follows:
...
\begin{definition}{\textbf{Halting Computation and time-t Halting Set}}
  ...
  is a sequence of configurations: $$\compute_x = (z_0, z_1, \ldots
  z_t)$$ such that \\
...
Here we sketch out some results that characterize the computable sets
over $R$.  Our first theorem shows that any computable set can be
constructed from a countable union of basic semi/quasi algebraic sets.
Before we proceed, we need a few more definitions.
  ...
  \begin{definition}{\textbf{Semi-algebraic} and \textbf{Quasi-algebraic Sets}}
    ...
    unordered ring, $R$, if $S$ is the set of elements in $R$
    satisfying a finite system of polynomial equalities. A set is
  ...
  \begin{definition}{\textbf{Coincidence set}}
    ...
    for some $x \in \inspace_M$. The \textbf{coincidence set} of $M$
    with respect to $\computepath$, is the set of elements in
    $\inspace_M$ sharing the same initial computation path of length
    $k$.  $\coincidence{\computepath}$, is given by:
    $$\coincidence{\computepath} = 
    \set{p' \in \inspace_M \mid \computepath_{x'}(k) = \computepath}$$
    
    \emph{\note{Coincidence sets partition $\inspace_M$ into
        equivalence classes.}}
...
\begin{definition}{\textbf{Size}}
  ...
  $$length(x) * \max_{1 \leq i \leq n}(ht_R(x_i))$$.
...
cost of performing a computation on a given machine, which we define
as the number of steps the machine takes before halting, times the
maximum size of any value stored in the machine's state space during
computation.
...
\begin{definition}{\textbf{Computation Cost}}
  ...
  computation on $x$.  In the case that $M$ does not halt on input
  $x$, we say that $cost_M(x) = \infty$.
