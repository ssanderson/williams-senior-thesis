...
Method, determining membership in the Mandelbrot Set, and the the
problem of determining whether a finite set of polynomials in $n$
...
their model.  In this thesis, we present a basic development of their
model as well as a sample of their major results.  In particular, we
show how Blum et al. define the relativized complexity classes, $\p$
and $\np$, which generalize the well-known classes $\mathbf{P}$ and
$\mathbf{NP}$ to machines that compute with values from an arbitrary
ring $R$.  We then develop a third complexity class, $\ndetp$, which
we present as an alternative generalization of the classical
$\mathbf{NP}$.  Our major results are that, over any ring $R$, $\p
\subseteq \ndetp \subseteq \np$ and $\ndetp \subseteq \exptime$. From
these containments it follows that $\p \subset \ndetp$ or $\ndetp
\subset \np$, then $\p \neq \np$.  We also demonstrate the existence
of at least one ring over which $\ndetp \subset \np$.  (It is already
known that $\p \neq \np$ for this case.)  We conclude by proving a
sufficient condition under which the equality $\ndetp = \np$ holds,
and we conjecture that this condition is also necessary.  
...
\begin{definition}{Turing Machine}
  ...
  into state $q'$, writes $\sigma'$ to the current cell, and moves the
...
\begin{definition}{\textbf{Finite Dimensional Machine} over a ring
  ...
  \item Associated with each computation node $\eta$ qis a
