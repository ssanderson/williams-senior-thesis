\chapter{Background - Finite Dimensional Machines}
\label{chap:background}

% In this chapter, we introduce the reader to the basic concepts of an
% algebraic model of computation due to Blum et al.  The material
% presented here draws heavily from \cite{B89} and \cite{B98}.  Since
% the computing machines developed by Blum et al. are essentially a
% generalization of the classical Turing Machine, we begin with a brief
% review of Turing Machines.  For a more thorough introductory reference
% to Turing Machines, see \cite{S06}.  For Turing's original article
% introducing the concept of a computing machine, see \cite{T36}.
...
%       \set{\leftarrow, \rightarrow}} is the machine's
...
% \note{The above definition is subject to modification
...
%   \end{center}
% \end{minipage}
% \captionof{figure}{Newton Machine - High Level} \label{fig:newton-high}
% \bigskip
...
%   is said to be \textbf{accepting} if $\eta = \accept$, and it is said
%   to be \textbf{rejecting} if $\eta = \reject$.  A configuration is
%   said to be \textbf{terminal} if it is either accepting or rejecting,
%   and it is said to be \textbf{non-terminal} otherwise.
...
%   \begin{align*}
%     &z_0 = (\eta_1, I(x))\\
%     &\pathprojection(z_t) \in \set{(\accept, \reject)}\\
%     &z_i \yields z_{i+1} \text{ for } 0 \leq i < t.
%   \end{align*}
  
%   Here the function \functype{\pathprojection}{\nodes \times
%     \statespace}{\nodes} denotes the projection onto the first
%   coordinate of a configuration.  Thus $\pathprojection(z)$ is just
%   the node occupied by a machine in configuration $z$, so the second
%   condition above says that the final configuration of a
%   halting computation must be terminal.\\
...
%     &x \in \halting_M(t)\\
%     &T_M(x) = t\\
%     &\computefn_M(x) = O(\stateprojection(z_t)).
%   \end{align*}

%   In plain english, the above expressions say that:
%   \begin{itemize}
%   \item The input $x$ is in the \textbf{time-$t$ Halting Set} of $M$, $\halting_M(t)$.
%   \item The \textbf{halting time} of $M$ on $x$, $T_M(x)$, is $t$.
%   \item The \textbf{output} of $M$ on $x$ is
%     $O(\stateprojection(z_t)$.
%   \end{itemize}

%   Here the function \functype{\stateprojection}{\nodes \times
%     \statespace}{\statespace} denotes the projection onto the second
%   coordinate of a configuration.  Thus the third statement above says
%   that the output of $M$ on $x$ is the result of applying the output
%   function $O$ to whatever is ``left behind'' in the statespace of $M$
%   in the last configuration.
  
%   The \textbf{Halting Set} $\halting_M$ of a machine $M$ is the union
%   over all $t > 0$ of $\halting_M(t)$.
...
%   node.  In such a case, $T_M(x)$ and $\computefn_M(x)$ are undefined,
%   and we define $\compute_M(x)$ to be the infinite sequence $$z_0,
%   z_1, z_2, \ldots, $$ such that
...
% $g_\eta(x)$, where $x$ is the current value stored in the machine's
...
%     \node(magcheck) [branch, draw, below of=init, label=135:\Large $\eta_2$] {$h(x) = x_z - x_1$};
%     \node(equal)    [branch, draw, right of= magcheck, node distance= 5.2cm, label=above:\Large $\eta_3$] {$h(x) = x_1 - x_2$};
...
%     \node (legend) [rectangle, draw, fill=black!10, below of=accept, node distance=3.0cm] {
%       \makecell[l]{
%         $\nodes = \set{\eta_1, \eta_2, \eta_3, \eta_4, \accept, \reject}$\\
%         $\inspace_M = \reals, \statespace_M = \reals^2, \outspace_M = \reals$\\
%         Computation Nodes: $\set{\eta_4}$\\
%         Branch Nodes: $\set{\eta_2, \eta_3}$\\
%         $\halting_M = \naturals$\\
%         $\computefn_M(x) = \ceil{x}$
%       }
%     };

%   \end{tikzpicture}
%   \caption{Decision Machine for $\naturals$}
%   \label{fig:z-decide}
% \end{figure}
...
%   input $x$ is the image of $\compute_M(x)$ under the map:
%   $$\mfunctype{\pathprojection}{\fullstate}{\nodes} \mid \pathprojection(\eta, x) = \eta.$$
  
%   The \textbf{state trajectory} of $M$ on input $x$ is the image
...
%   \computefn_M(x) \mid \forall x \in X$.
...
% \cite{B98} and \cite{B89}.  Blum et al. do not explicitly appeal to
...
% decidability.  It is straightforward to verify that thes classes of
...
% \begin{example}
%   The machine presented in Figure \ref{fig:q-recog} shows that
%   $\rationals$ is finite-dimensionally recognizable over $\reals$.
%   The machine presented in Figure \ref{fig:z-decide} shows that
%   $\integers$ is finite-dimensionally decidable over $\reals$.
% \end{example}
...
%   recognizer for both $S_{yes}$ and $S_{no}$ .  Conversely, if $M$
%   and $M'$ are recognizers for $S$ and $R^n - S$, we can contruct a
%   decision machine for $S$ by simulating $M$ and $M'$ in parallel,
%   accepting if $M$ accepts and rejecting if $M'$ accepts.  Since one
%   of $M$ and $M'$ must halt eventually, we are guaranteed that our
%   new machine will halt for all possible inputs, and will accept an
%   input if an only if it is in $S_{yes}$.
...
%     traverse different sequences of nodes.  It is straightforward to
%     show that belonging to the same coincidence set of length $k$
%     defines an equivalence relation on elements of $\inspace_M$.
%     Thus, for each $k > 0$, the coincidence sets of $M$ on paths of
%     length $k$ partition $\inspace_M$ into equivalence classes.}
...
%   We can use this corollary to prove the following:
...
%   \section{Computing with Unbounded State}
...
%       map \functype{\widehat{g}_\eta}{R_\infty}{R_\infty}. We denote
...
%       \functype{\sigma_\eta}{R_\infty}{R_\infty}, which is either the
...
%     above.  For a justification of this approach, see Sections 3.5 and
%     4.4 of \cite{B98}.}
...
%     One cause for care is that the shift node maps are not
%     polynomial when considered as maps from $R_\infty \rightarrow
%     R_\infty$.  This difficulty is resolved by noting that the shift
%     maps can be reconstructed as polynomial maps for any
%     finite-dimensional subset, $R^n \subset R_\infty$, and by noting
%     that in time $T$ the machine can only modify a finite number of
%     registers.  This follows from the fact that all the computation
%     maps are extensions of finite polynomial maps from $R^m
%     \rightarrow R^n$, and from the fact that a given computation can
%     traverse at most T shift nodes in time T.  Since we can bound
%     the total number of coordinates in $\statespace_M$ accessible by
%     a given machine in time $T$, the computation performed by $M$ in
...
%     A \textbf{Decision Problem} over a ring $R$ is a set, $S
%     \subseteq R^\infty$, partitioned into subsets, $S_{yes}$ and
%     $S_{no}$, respectively called the \textbf{Yes Instances} and
%     \textbf{No Instances} of $S$.
...
% \section{Measuring the Cost of Computation}
% \label{sec:measuring}
...
% associated with the ``magnitude'' of entries in $\inspace_M$ or
% $\statespace_M$.  Drawing on terminology from algebraic number theory,
% we refer to this latter notion as the \textbf{height} of an entry.  We
% then define the overall \textbf{size} of an input in terms of both its
% length and the heights of its entries.

% \begin{definition}{\textbf{Height Function} over $R$}

%   A \textbf{Height Function} on a ring $R$ is a map:
...
%   an element $x \in R^n$ is given by:
...
% machine takes before halting \emph{times} the maximum height of any
% value stored in the machine's state space during computation.
...
%   state trajectory of $M$ on input $x$ is given by:
%   \begin{eqnarray*}
...
%   Chapter 4 of \cite{B98}.  Blum et al. use the notation $\cost_M(x)$,
%   rather than $\cost(\compute_M(x))$.  Our change reflects our desire
%   to emphasize that cost is determined by a height function combined
%   with a particular path through the configuration space of $M$.  This
%   will be important for us in Chapter \ref{chap:ndet} when we consider
%   machines that traverse multiple paths on a single input.}
...
%   $\dtime(O(f(x)))$ (deterministic time $O(f(x))$) if there exists a
%   machine $M$ and a constant $c \in \reals$ such that $M$ decides $S$
%   and for all $x \in \inspace_M$ we have:
...
% \begin{example}
%   \todo{Nonlinear example for this.}
% \end{example}

% \begin{definition}{\textbf{Polynomial Time Machine}}

%   A machine $M$ over $R$ is a \textbf{polynomial time} machine on
%   inputs $X \subset \inspace_M$ if there exist positive integers $c$
%   and $q$ such that for all $x \in X$:
%   $$cost_M(x) \leq c(size(x))^q.$$
% \end{definition}

% \begin{definition}{\textbf{p-morphism}}
  
%   A function, \functype{\varphi}{X}{Y \subset R^\infty} is called
...
%   \note{Since compositions of polynomials are still polynomials, it
%     immediately follows that if $S \in \p$, and $S \preduce S'$, then
%     $S' \in \p$, so $\p$ is closed under the relation of
%     p-reducibility. It is similarly easy to show that if $X \preduce
%     Y$ and $Y \preduce Z$, then $X \preduce Z$ so the relation of
%     p-reducibility is transitive.}
...
%     \end{enumerate}
%   \end{enumerate}
%   References for all of the above results can be found in \cite{B98}.
% \end{theorem}

% \section{The Class $\np$}

% The definition of $\np$ given by Blum et al is a generalization of the
% classical characterization of $\mathbf{NP}$ as the class of decision
% problems which are verifiable in polynomial time.  The idea of
% verifiability is roughly the following: for many decision problems
% $S$, it seems difficult to determine whether an arbitrary input $s$ is
% a member of $S$.  For some of these problems, however, there exist
% ``short'' proofs of the fact that $s \in S$.\\
...
%   class $\np$ if there exist positive integers $c$ and $q$ and a BSS
%   Machine $M$ over R with $\inspace_M = R^\infty \times R^\infty$ such
%   that:

%   \begin{enumerate}
%   \item If $x \in S$ then there exists a $w \in R^\infty$, such that
...
%   The decision problem $\hn$ is a member of $\np$ for $R = \reals$, $R
%   = \complexes$, and $R = \integers$ with unit cost.  Recall that an
%   instance of $\hn$ is an encoding of some of polynomials $S =
%   \set{p_1, p_2, \ldots, p_k} \subset \polyn{R}$, and that an instance
%   is a Yes Instance if there exists some $x \in R^n$ such that for
%   each $p_i \in S$, $p_i(x) = 0$.  To show that $\hn \in \np$, we must
%   exhibit a machine which takes as inputs pairs $(w, x)$, and for each $x$

%   \todo{here}

%   $(w, x)$, $w \in R^n$ and $x =
%   \langle \set{p_1, p_2, \ldots, p_m} \subseteq \polyn{R} \rangle$ and
%   evaluates each $p_i$ encoded by $x$ at the point $w$.  Our machine
%   accepts if all the $p_i$ evaluate to $0$, and rejects
%   otherwise. \footnote{In the case that $x$ encodes a system of
%     polynomials defined over $k < n$ variables, we can simply evaluate
%     each polynomial over the first $k$ coordinates of $w$.  If $x$
%     encodes polynomials of dimension greater than $n$, we simply
%     reject.}
...
% \todo{Figure out what to do with this and the section on decision
%   problems.}
