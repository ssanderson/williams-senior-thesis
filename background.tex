...
%   \todo{Should $\halting_M(t)$ be things that halt at \emph{or before} $t$ ?}
...
%   \begin{corollary}
%     If $S \subset R^n$ is finite dimensionally decidable, then both
%     $S$ and $R^n -S$ are countable unions of disjoint,
...
%     Let \functype{h}{R^m}{R} be a polynomial map.  Then $h$ defines a
%     related polynomial map \functype{\widehat{h}}{R_\infty}{R}, which
%     is given by:
...
%     polynomial map from $R_m$ to $R$.  Let $x \in R_\infty = \ldots
%     x_{-2}x_{-1}x_0.x_1x_2 \ldots$. We define the associated
%     polynomial map \functype{\widehat{g}}{R_\infty}{R_\infty} as
%     follows:
%     $$\widehat{g}(x) = 
%     \ldots x_{-2}x_{-1}x_0.
...
%     the notation $\langle expression \rangle$ to denote an
%     \textbf{encoding} of $expression$.

%     \begin{algorithm}
%       \caption{Newton's Method} \label{alg:newton}
%       \begin{algorithmic}
%         \Require $\langle p(x), x_0 \rangle \mid p(x) \in \poly{R}\,; x_0 \in R\,; \epsilon \in R$
%         \State Compute $\langle p'(x) \rangle$
%         \While{$p(x_0) > \epsilon$ }
...
%     statements then follow from Theorem \ref{thm:ptd-finite}.
